...
### 角色策略（详见 docs/09 第 2.2 与 14 节）

- P0：主控角色基础行走（复用现有 12 帧动画与移动逻辑），花园内自由走动；
- P0：走到入口区域触发场景切换；入口木牌同时支持直接点按（手机 P0 主路径）；
- P1：手机虚拟摇杆/点击移动、靠近入口提示气泡；
- P2：多角色行走、复杂动作（比赛版不做）。

### 花园入口表

| portal_id | 位置（逻辑坐标区域） | target_scene | spawn_point | prompt_text | 优先级 |
|---|---|---|---|---|---|
| garden_portal_fishpond | 右侧水边 [1120, 384, 96, 128] | fishpond | from_garden | 进入爸爸鱼塘 | P0 |
| garden_portal_room | 左侧房门 [128, 384, 96, 128] | room | from_garden | 回到我的房间 | P0 |
| garden_portal_farm | 上方小路 [576, 128, 128, 80] | farm | from_garden | 去爷爷农场 | P1 |
| garden_portal_oldstreet | 远景巷口（背景定稿后定） | old_street | from_garden | 走进老街 | P1 |

### 花园 spawn points

| spawn_id | 坐标 | 用途 |
|---|---|---|
| default | [640, 480] | 首次进入/兜底 |
| from_fishpond | [1056, 448] | 从鱼塘返回 |
| from_room | [224, 448] | 从房间返回 |
| from_farm | [640, 224] | 从农场返回 |

坐标均为 1280×720 逻辑坐标，背景定稿后由 A+C 校准一次。字段与触发规则见 `docs/09_godot_coordinate_art_spec.md` 第 14 节。

### 验收追加

- 角色能在花园行走并被前景树冠正确遮挡；
- 走到鱼塘入口能进入鱼塘，从鱼塘返回后出现在 from_fishpond 位置；
- 手机端在没有移动控件时，点按入口木牌也能完成同样的场景切换。
