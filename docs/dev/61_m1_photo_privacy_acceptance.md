...
>
> 状态：M1 主链路及 2026-08-02 审查修复均已完成生产部署、线上校验与真实烟测
...
- [x] 2026-08-02 审查修复复部署：线上源码哈希一致，函数配置无漂移，真实烟测再次全部通过。
...
- 兼容修复部署时间：2026-07-31 17:22（CST）；
- 审查修复部署时间：2026-08-02 20:04（CST）；
...
2. 云函数最终修改时间为 `2026-08-02 20:04:11`，状态为 `Active/Available`；
3. 从云端回下载 `index.js`，与本地文件的 SHA-256 均为
   `5f391cefca3c371361d43f31c58c1f737627cb72177f22557228366353cf8a0b`；
4. 云端回下载的 `package.json` 和 lockfile 均锁定 `@cloudbase/node-sdk=3.18.3`；
5. 现有函数继续使用 `Nodejs18.15`、15 秒超时、256 MB 内存及云端自动安装依赖，代码更新
   未造成函数配置漂移。腾讯 SCF 的既有函数运行时不能通过配置接口原地修改；
...
2026-08-02 审查修复复部署后，以唯一家庭标识
`family1_m1_photo_20260802120509_b47322`（隔离家庭追加 `_isolated`）再次执行上述烟测，所有步骤
通过，命令退出码为 0。

### 6.2 清理复核

2026-08-02 烟测结束后，以本次两个唯一临时家庭标识直接查询云数据库：
...
- `postcards`：0；
- `mailbox_events`：0；
- 私有对象：级联删除成功，删除后 `resolve_image` 返回 404，`uploads` 元数据为 0。

本轮生成的 3 条测试成员已在核对家庭标识和 `M1 Photo` 显示名后按精确 `_id` 删除；删除结果
`n=3`，随后复查上述 5 个集合均为空，没有删除正式家庭成员。此前部署轮次的 6 条测试成员也已
按精确 `_id` 清理。

## 7. 后续运维边界

- M1 新照片链路已可用；2026-08-02 的失败关闭与分页修复已随 PR 合入，并完成生产部署、源码
  哈希校验与真实烟测；
