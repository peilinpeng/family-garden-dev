...
func _build_action_dock() -> void:
	...
	_dock.position = Vector2(382, 652)
	_dock.size = Vector2(516, 54)
	...
	labels.text = "地图       背包       聊天"
	labels.position = Vector2(178, 36)
	labels.size = Vector2(180, 14)
	...
	shortcut.position = Vector2(362, 17)
...
func set_context(scene_id: String) -> void:
	...
	if _dock != null:
		_dock.position.x = 382.0 if in_garden else 545.0
		_dock.size.x = 516.0 if in_garden else 190.0
